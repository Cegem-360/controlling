<x-layouts.app>
        ...
        <div class="container mx-auto px-6 py-4">
            <div class="flex items-center justify-between">
                ...
                <nav class="hidden lg:flex lg:items-center lg:gap-2">
                    ...
                    <a href="/admin"
                        class="px-6 py-2 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-all ml-2">{{ __('Admin Panel') }}</a>
                </nav>
            </div>
        </div>
    </header>
    ...
    <section class="py-24 px-6 min-h-screen flex items-center justify-center bg-gray-900 text-white">
        <div class="container mx-auto max-w-4xl text-center">
            ...
            <div class="flex gap-4 justify-center">
                <a href="#contact"
                    class="inline-flex items-center px-8 py-4 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-all shadow-xl hover:shadow-2xl">
                    {{ __('Get Started') }}
                </a>
                <a href="#features"
                    class="inline-flex items-center px-8 py-4 bg-white text-gray-900 font-semibold rounded-lg hover:shadow-2xl transition-all">
                    {{ __('Learn More') }}
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div class="bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all">
                    ...
                    <p class="text-gray-600 font-light">
                        {{ __('Monitor your key performance indicators in real-time with intuitive dashboards and visualizations.') }}
                    </p>
                </div>
                <div class="bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all">
                    <h3 class="text-2xl font-bold mb-4 text-purple-600">{{ __('Data Integration') }}</h3>
                    <p class="text-gray-600 font-light">
                        {{ __('Seamlessly connect with multiple data sources including Google Analytics, databases, and APIs.') }}
                    </p>
                </div>
                <div class="bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all">
                    <h3 class="text-2xl font-bold mb-4 text-purple-600">{{ __('Custom Reports') }}</h3>
                    <p class="text-gray-600 font-light">
                        {{ __('Create custom reports tailored to your business needs with our flexible reporting engine.') }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <section id="contact" class="py-20 px-6 bg-white">
        <div class="container mx-auto max-w-2xl">
            ...
            <form action="#" method="POST" class="space-y-6">
                ...
                <div>
                    <label for="company"
                        class="block text-sm font-medium text-gray-700 mb-2">{{ __('Company') }}</label>
                ...
                <div>
                    <label for="message"
                        class="block text-sm font-medium text-gray-700 mb-2">{{ __('Message') }}</label>
                ...
                <button type="submit"
                    class="w-full px-8 py-4 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-all shadow-lg hover:shadow-xl">
                    {{ __('Send Message') }}
                </button>
            </form>
        </div>
    ...
    <footer class="py-8 px-6 bg-gray-900 text-white">
        <div class="container mx-auto max-w-7xl text-center">
            <p class="text-gray-400 font-light">&copy; {{ date('Y') }} {{ config('app.name') }}.
