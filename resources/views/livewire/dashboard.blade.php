<div>
    {{-- Page header --}}
    <div class="mb-8">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white font-heading">Üdvözöljük, {{ auth()->user()->name }}!</h1>
    ...
    @if($teams->count() > 0)
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="p-6 border-b border-gray-100 dark:border-gray-700">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-semibold">
                                    {{ strtoupper(substr($team->name, 0, 1)) }}
                                </div>
                                <div>
                                    <h3 class="font-semibold text-gray-900 dark:text-white">{{ $team->name }}</h3>
                                    @if($team->settings?->site_url)
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $team->settings->site_url }}</p>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-orange-600 dark:text-orange-400" viewBox="0 0 24 24" fill="none">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="none">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-green-600 dark:text-green-400" viewBox="0 0 24 24" fill="none">
            ...
            <a href="{{ route('filament.admin.tenant.registration') }}"
                class="flex flex-col items-center justify-center min-h-[280px] bg-white dark:bg-gray-800 rounded-2xl shadow-sm border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-emerald-500 dark:hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-all group">
                <div class="w-16 h-16 rounded-2xl bg-gray-100 dark:bg-gray-700 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/30 flex items-center justify-center mb-4 transition">
                    <svg class="w-8 h-8 text-gray-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                </div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Új csapat létrehozása</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 text-center max-w-[200px]">Adjon hozzá új weboldalt a kontrolling rendszerhez</p>
            </a>
        </div>
    @else
        {{-- Empty state --}}
        <div class="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p class="text-gray-600 dark:text-gray-400 text-center max-w-md mb-8">
                Hozza létre első csapatát, és kezdje el a weboldalai teljesítményének követését a Google Analytics, Search Console és Google Ads adatokkal.
            </p>
            <a href="{{ route('filament.admin.tenant.registration') }}"
