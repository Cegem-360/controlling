<div>
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('All Pages') }}</h3>
                    <div class="relative">
                        <input
                            type="text"
                ...
                @if(count($paginatedPages['data']) > 0)
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50 dark:bg-gray-900/50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">
                                        <button wire:click="sort('page_url')" class="flex items-center gap-1 hover:text-gray-700 dark:hover:text-gray-200 cursor-pointer">
                                            {{ __('Page URL') }}
                                            @if($sortBy === 'page_url')
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    @if($sortDir === 'asc')
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($paginatedPages['data'] as $page)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300 whitespace-nowrap">{{ $page['date'] }}</td>
                                        <td class="px-4 py-3 text-gray-900 dark:text-white font-medium truncate max-w-xs" title="{{ $page['page_url'] }}">{{ $page['page_url'] }}</td>
                                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $page['device'] }}</td>
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        @if($paginatedPages['lastPage'] > 1)
                            <div class="flex items-center gap-2">
                                <button
                                    wire:click="$set('page', {{ max(1, $paginatedPages['currentPage'] - 1) }})"
                                ...
                                <button
                                    wire:click="$set('page', {{ min($paginatedPages['lastPage'], $paginatedPages['currentPage'] + 1) }})"
    ...
    @else
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            <p class="text-gray-600 dark:text-gray-400 mb-4">{{ __('You need to create a team before viewing Search Pages.') }}</p>
